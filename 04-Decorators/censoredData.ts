  ...
  return function (
    ...
    descriptor.value = function () {
      ...
      const recentItems = allItems.filter(
        (item: any) =>
          currentTime.getTime() - item.createdOn.getTime() <= seconds * 1000
      );
      return recentItems;
    };
...
function censoreProps<T extends { [key: string]: any }>(
...
class UsersService {
  ...
  @getLatest(5)
  @censoreProps(userCensorService)
  getUsers() {
    return this._users;
  }
  ...
  @log
  @getLatest(10)
  @censoreProps(employeeCensorService)
  getEmployees() {
